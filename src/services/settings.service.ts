...
export class SettingsService implements OnDestroy {
  ...
  getAllSettingsQuery() {
    return injectQuery(() => ({
      ...
      queryFn: async () => {
        const { data, error } = await this.supabase.client.from(DB_TABLES.SETTINGS).select('*').order('key');
  ...
  deleteSettingMutation() {
    return injectMutation(() => ({
      mutationFn: async (id: string) => {
        const { error } = await this.supabase.client.from(DB_TABLES.SETTINGS).delete().eq('id', id);
