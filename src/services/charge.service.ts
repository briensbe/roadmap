...
export class ChargeService {
  ...
  async bulkMoveCharges(
    ...
    if (toInsert.length > 0) {
      const { error: insertError } = await this.supabase.client.from(DB_TABLES.CHARGES).insert(toInsert);
  ...
  async getAvailableRolesForProjectTeam(projetId: string, equipeId: string): Promise<Role[]> {
    ...
    const usedRoleIds = new Set((chargesInCombination || []).filter((c) => c.role_id).map((c) => c.role_id));
  ...
  async getAvailablePersonnesForProjectTeam(projetId: string, equipeId: string): Promise<Personne[]> {
    ...
    const usedPersonneIds = new Set(
      (chargesInCombination || []).filter((c) => c.personne_id).map((c) => c.personne_id),
    );
  ...
  async deleteChargesForResource(
    ...
    let query = this.supabase.client.from(DB_TABLES.CHARGES).delete().eq('equipe_id', equipeId);
  ...
  async deleteChargesForProjectTeam(projetId: string, equipeId: string): Promise<void> {
  ...
  private getFirstDayOfWeek(fromDate: string) {
    ...
    firstDayOfWeek.setDate(inputDate.getDate() - inputDate.getDay() + (inputDate.getDay() === 0 ? -6 : 1));
